    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['biz:authority:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['biz:authority:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listAuthority, getAuthority, delAuthority, addAuthority, updateAuthority, exportAuthority } from "@/api/biz/authority";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAuthority(keyId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除调用key编号为"' + keyIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delAuthority(keyIds);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有调用key数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportAuthority(queryParams);
        }).then(response => {
